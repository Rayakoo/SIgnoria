...
// ==== TYPE DEFINITIONS UNTUK MEDIAPIPE ====
declare global {
  interface Window {
    Hands: any;
    Camera: any;
...
export default function AlphabetCam() {
  ...
  const predictionEndpoint = "https://signoria.gilanghuda.my.id/api/predict/alphabet";
  ...
  const sendVectorToServer = async (vector: number[]) => {
    try {
      ...
      if (data.error) {
        console.error("API error:", data.error); // Log any error returned by the API
        setError(data.error);
        return;
      }
