...
interface ProgressData {
  overall_summary: {
    total_sublevels: number;
    completed_sublevels: number;
    completion_rate: number;
    total_stars: number;
    total_attempts: number;
    average_score: number;
  };
  progress_by_level: Record<string, Level>;
}
...
export default function JourneyPage() {
  ...
  const [skipLevelId, setSkipLevelId] = useState<number | null>(null);
  ...
  const levels = useMemo(() => {
    ...
    return Object.entries(data.progress_by_level) as [string, Level][];
  }, [data]);

//   const skipLevel = async (levelId: number) => {
//     try {
//       const token = typeof window !== "undefined" ? localStorage.getItem("token") : null;
//       if (!token) return;
...
//   };
                  ...
                  {lvl.sublevels.map((sub) => (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center rounded-md sm:rounded-lg md:rounded-xl lg:rounded-2xl z-10">
                        <Lock size={16} className="sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 text-[#ADF5FF]" />
                      </div>
                    </div>
                  ))}
                </div>
 ...
 // Ensure the callback does not return a value
              className="space-y-3 sm:space-y-4 md:space-y-5 lg:space-y-6"
            >
              {/* Header Level */}
              <div className="rounded-md sm:rounded-lg md:rounded-xl lg:rounded-2xl border border-gray-100 bg-gradient-to-r from-white to-gray-50 shadow-sm hover:shadow-md transition-all duration-300 p-3 sm:p-4 md:p-5 lg:p-6">
