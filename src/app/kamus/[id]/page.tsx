...
import { fetchKamusDetail } from "@/services/kamusservice";
import MediaTabs from "@/components/kamus/MediaTabs";
import MediaContent from "@/components/kamus/MediaContent";
import InfoItem from "@/components/kamus/InfoItem";
import ActionButtons from "@/components/kamus/ActionButtons";

export default function DictionaryDetail({ params }: { params: { id: string } }) {
  const { id } = params;

  const { data, error, isLoading, mutate } = useSWR(
    id ? `/public/kamus/${id}` : null,
    fetchKamusDetail
  );
    ...
    <div className="min-h-screen bg-white px-6 py-10 font-poppins transition-all duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-4">
          <MediaTabs tab={tab} setTab={setTab} hasVideo={!!data.video_url} />
          <MediaContent
            tab={tab}
